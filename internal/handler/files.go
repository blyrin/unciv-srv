...
func GetFile(w http.ResponseWriter, r *http.Request) {
	...
	if isPreview {
		...
		if err != nil {
			slog.Error("获取预览存档失败", "gameId", gameID, "error", err)
			utils.ErrorResponse(w, http.StatusInternalServerError, "获取存档失败")
			return
		}
		if file == nil {
			utils.ErrorResponse(w, http.StatusNotFound, "找不到存档")
			return
		}
		data = file.Data
	} else {
		...
		if err != nil {
			slog.Error("获取存档失败", "gameId", gameID, "error", err)
			utils.ErrorResponse(w, http.StatusInternalServerError, "获取存档失败")
			return
		}
		if file == nil {
	...
	if err != nil {
		slog.Error("编码存档失败", "gameId", gameID, "error", err)
		utils.ErrorResponse(w, http.StatusInternalServerError, "编码存档失败")
		return
	}
...
func PutFile(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("解码存档失败", "playerId", playerID, "gameId", gameID, "error", err)
		utils.ErrorResponse(w, http.StatusBadRequest, "存档格式无效")
		return
	}
	...
	if err != nil {
		slog.Error("解析游戏数据失败", "playerId", playerID, "gameId", gameID, "error", err)
		utils.ErrorResponse(w, http.StatusBadRequest, "存档格式无效")
		return
	}
	...
	if gameData.GameID != gameID {
		...
		utils.ErrorResponse(w, http.StatusBadRequest, "游戏ID不匹配")
		return
	}
	...
	if err != nil {
		slog.Error("获取玩家列表失败", "playerId", playerID, "gameId", gameID, "error", err)
		utils.ErrorResponse(w, http.StatusBadRequest, "无法获取玩家列表")
		return
	}
	...
	if !slices.Contains(playerIDs, playerID) {
		...
		utils.ErrorResponse(w, http.StatusForbidden, "你不是该游戏的玩家")
		return
	}
	...
	if err != nil {
		slog.Error("获取游戏失败", "gameId", gameID, "error", err)
	...
	if game == nil {
		...
		if err := database.CreateGame(ctx, gameID, playerIDs); err != nil {
			slog.Error("创建游戏失败", "gameId", gameID, "error", err)
			utils.ErrorResponse(w, http.StatusInternalServerError, "创建游戏失败")
			return
		}
	} else {
		// 验证玩家权限
	...
	if isPreview {
		if err := database.SaveFilePreview(ctx, gameID, turns, playerID, ip, decodedData); err != nil {
			slog.Error("保存预览存档失败", "gameId", gameID, "error", err)
