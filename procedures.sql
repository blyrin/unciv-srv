...
-- ========================================
-- 管理员认证相关存储过程 / Admin Authentication Stored Procedures
-- ========================================
-- 验证管理员凭据存储过程
-- Validate admin credentials stored procedure
CREATE
OR REPLACE FUNCTION "sp_validate_admin_auth" (
...
BEGIN
    -- 检查管理员用户名和密码是否配置 / Check if admin username and password are configured
    IF "p_admin_username" IS NULL OR "p_admin_password" IS NULL OR
       length("p_admin_username") = 0 OR length("p_admin_password") = 0 THEN
        -- 如果管理员凭据未配置，返回非管理员状态 / If admin credentials not configured, return non-admin status
        RETURN QUERY SELECT "p_username", "p_password", false;
        RETURN;
    END IF;

    -- 验证提供的凭据 / Validate provided credentials
    IF "p_username" = "p_admin_username" AND "p_password" = "p_admin_password" THEN
        -- 凭据匹配，返回管理员状态 / Credentials match, return admin status
        RETURN QUERY SELECT "p_username", "p_password", true;
    ELSE
        -- 凭据不匹配，返回非管理员状态 / Credentials don't match, return non-admin status
        RETURN QUERY SELECT "p_username", "p_password", false;
    END IF;
END;
$$;
